...
const gorevler = [
  ...
  G("Çözgü B mekik ayarı", { tz: "t1", cz: "c2", onc: 0 }),
  G("Tezgah geneli bakım", { tz: "t1" }),
  G("Çözgü D leno kontrol", { tz: "t2", cz: "c4", tarih: "2026-07-10" }),
];
function ver(path) {
...
for (const [w, etiket] of [[1440, "tablet"], [1920, "masaustu"]]) {
  ...
  // 1) Sade hâl — kapalı ekleme çubuğu, boş çözgü gizli, çipler sade
  await page.screenshot({ path: `scripts/gorev-sade-${etiket}.png`, fullPage: false });
  // 2) Detay açık + boş çözgü göster
  await page.click(".gorev-detay-ac");
  await page.check(".gorev-filtre:has-text('Boş çözgüleri') input");
  await page.waitForTimeout(200);
  await page.screenshot({ path: `scripts/gorev-sade-detay-${etiket}.png`, fullPage: false });
  // 3) Satır hover → araçlar belirir
  await page.uncheck(".gorev-filtre:has-text('Boş çözgüleri') input");
  await page.click(".gorev-detay-ac");
  const satir = page.locator(".gorev-satir").first();
  await satir.hover();
  await page.waitForTimeout(200);
  await page.screenshot({ path: `scripts/gorev-sade-hover-${etiket}.png`, fullPage: false });
  await page.close();
}
console.log("yazıldı");
await browser.close();
